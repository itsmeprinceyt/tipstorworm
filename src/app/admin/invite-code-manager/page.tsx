...
export default function InviteTokensPage() {
  ...
  const fetchTokens = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get<InviteTokenResponseDTO>(
        "/api/admin/invite-code-manager"
      );
  ...
  const disableToken = async (tokenString: string) => {
    try {
      const response = await axios.post<SuccessResponseDTO>(
        "/api/admin/invite-code-manager/disable",
        {
          token: tokenString,
        }
      );
      ...
      if (response.data.success) {
        ...
        setTokens((prevTokens) =>
          prevTokens.map((token) =>
            token.token === tokenString ? { ...token, active: false } : token
          )
        );
      }
    } catch (err: unknown) {
      setTokens((prevTokens) =>
        prevTokens.map((token) =>
          token.token === tokenString ? { ...token, active: true } : token
        )
      );
  ...
  const getTimeUntilExpiry = (dateString: string) => {
    ...
    if (diffMs <= 0)
      return ".. wait a second ! Select proper expiry date & time";
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-white">Invite Tokens</h1>
          ...
          >
            ...
            <div className="bg-black/40 backdrop-blur-sm border border-stone-800 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-emerald-400 mb-1">
                {
                  tokens.filter((t) => getStatusInfo(t).text === "Active")
                    .length
                }
              </div>
            ...
            <div className="bg-black/40 backdrop-blur-sm border border-stone-800 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-red-400 mb-1">
                {
                  tokens.filter((t) => getStatusInfo(t).text === "Expired")
                    .length
                }
              </div>
                    ...
                    {tokens.map((token, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div
                              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${statusInfo.borderColor} ${statusInfo.bgColor}`}
                            >
                              <StatusIcon
