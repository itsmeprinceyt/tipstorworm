        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
            {particleTypes.map((type, typeIndex) => (
                [...Array(type.count)].map((_, i) => {
                    ...
                    const duration = type.duration[0] + Math.random() * (type.duration[1] - type.duration[0]);
                    const distance = type.distance[0] + Math.random() * (type.distance[1] - type.distance[0]);
                    const delay = Math.random() * (type.delay[1] - type.delay[0]) + type.delay[0];
                    
                    const animateProps = type.movement === "horizontal" 
                        ? {
                    ...
                    return (
                        <motion.div
                            key={particleKey}
                            className={`absolute ${type.size} ${type.color} ${type.shape || 'rounded-full'}`}
                            initial={{
                                x: randomX + 'vw',
                                y: randomY + 'vh',
                                scale: 0,
